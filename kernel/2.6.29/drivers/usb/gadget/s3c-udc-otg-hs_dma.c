 ...
 *
 * ADB_UMS_ACM || ADB_UMS:
 *	EP2 => EP0 in reconfig_usbd, add EP0 => EP2 in setdma_tx
 *  							 add EP0 => EP3 in setdma_tx
 *	EP5 => EP0 in reconfig_usbd, add EP0 => EP5 in setdma_tx
 *	EP8 => EP0 in reconfig_usbd, add EP0 => EP8 in setdma_tx
 *
...
//Address offset between each ep
...
/**
 * del_one_ep_list_from_ep0_next_ep
...
static inline void s3c_ep_send_zlp(u32 ep_num)
{
	...
	DEBUG_IN_EP("[%s] for EP %d\n", __FUNCTION__, ep_num);

	ctrl = readl(S3C_UDC_OTG_DIEPCTL0+DEP_OFFSET*ep_num);

	writel((1<<DEPTSIZ_PKT_CNT_BIT)|(0<<DEPTSIZ_XFER_SIZE_BIT), (u32) S3C_UDC_OTG_DIEPTSIZ0+DEP_OFFSET*ep_num);
...
static void complete_tx(struct s3c_udc *dev, u32 ep_num)
{
	...
	if (req->req.actual == req->req.length) 
	{
		...
		if (req->zlp == true)
		{			
			printk("[%s] S3C_UDC_ZLP req->req.actual == req->req.length\n", __func__);
			//setdma_tx(ep, req);
...
#if 0 
//replaced by s3c_change_nextep() called at set configuration
	...
	{		
		...
		if(dev->config_gadget_driver == ANDROID_ADB_UMS ||
 ...
 */
static void s3c_change_nextep(struct s3c_udc *dev)
{
	DEBUG_SETUP("[%s] \n", __FUNCTION__);
...
// out of date 	WARN_ON(!list_empty(&req->queue));
	kfree(req);
}
//---------------------------------------------------------------------------------------
			 ...
			 gfp_t gfp_flags)
{
	...
	if(req->req.zero)
	{
		...
		{				
			req->zlp = true;
			printk("[%s] S3C_UDC_ZLP req->zlp = true\n", __func__);
		}				
	}
#endif	
...
/**
 * s3c_ep0_read_ctrl
 * read control req from fifo (8 bytes), setup transaction
 */
static inline int s3c_ep0_read_ctrl(struct s3c_ep *ep, u32 *cp, int max)
{
	u32 bytes;
...
static void s3c_ep0_setup(struct s3c_udc *dev)
{
	...
	bytes = s3c_ep0_read_ctrl(ep, (u32 *)&ctrl, 8);
...
#define USB_CDC_SET_ETHERNET_PACKET_FILTER	0x43
	...
	/* Handle some SETUP packets to udc*/
	switch (ctrl.bRequest) {
...
config_change:
		...
		case USB_REQ_GET_DESCRIPTOR:
			...
			/*
				CHECK
				Walkaround for failing to install UMS that only happend at first time.
				Host send a packet with wValue has 0x03EE
				EE is a String ID , there is no 0xEE ID
			*/
			if (ctrl.wValue == 0x03ee) 
			{
				printk("\n\n[%s] CHANGE ctrl.wValue 0x03EE => 0x0300 ~~~~\n\n", __FUNCTION__);
				ctrl.wValue = 0x0300;
			}
			break;
		...
		case USB_REQ_GET_STATUS:
		    ...
		    //need to implement real handling instead of just sending 0 value
...
gadget_setup:
	if (likely(dev->driver)) 
	{
		...
		if (i < 0) 
		{
			...
			printk("[%s] gadget setup FAILED (stalling), setup returned %d\n",
				__FUNCTION__, i);
			/* ep->stopped = 1; */
			dev->ep0state = WAIT_FOR_SETUP;
		}
